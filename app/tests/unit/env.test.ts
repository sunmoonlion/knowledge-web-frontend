...
describe('server environment', () => {
  it('uses the template identity by default outside production', () => {
    expect(parseServerEnv({ NODE_ENV: 'development' }).AUTH_APP).toBe('knowledge')
  })
  ...
  it('accepts an explicit production runtime contract', () => {
    expect(
      parseServerEnv({
        ...
        AUTH_APP: 'info',
        APP_ORIGIN: 'https://knowledge.sunmoonai.com',
  ...
  it.each([
    ['http application origin', { APP_ORIGIN: 'http://knowledge.sunmoonai.com' }],
    [
