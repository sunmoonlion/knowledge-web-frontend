...
describe('loadBrowserSession', () => {
  it('accepts the RFC 3339 UTC offset emitted by the FastAPI backend', async () => {
    ...
    await expect(
      loadBrowserSession({
        ...
        cookieHeader: 'sunmoonai_knowledge_web_sid=opaque-session',
  ...
  it('forwards only the cookie and correlation context and accepts the exact safe DTO', async () => {
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      new URL('http://knowledge-backend:8000/api/auth/web/me'),
      expect.objectContaining({
        cache: 'no-store',
        redirect: 'manual',
        headers: {
          Accept: 'application/json',
          Cookie: 'sunmoonai_knowledge_web_sid=opaque-session',
